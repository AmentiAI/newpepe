...
export default function NadAgeCalculator() {
  ...
  return (
    <div className="glass-card p-6 space-y-5">
      <div>
        <h3 className="text-white font-black text-lg mb-1">NAD+ Decline & System Impact Calculator</h3>
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <label className="text-slate-400 text-xs mb-1.5 block font-semibold">Your Age</label>
          <input
            type="number"
            min={20}
            max={80}
            ...
            className="w-full bg-dark-800 border border-slate-700 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500/60"
          />
        </div>
        <div className="flex-1 rounded-xl p-4 bg-amber-500/10 border border-amber-500/20 text-center">
          ...
          <div className="text-slate-400 text-xs mt-1">Estimated NAD+ Level</div>
          <div className="text-slate-500 text-xs">(vs age 20 baseline)</div>
        </div>
      </div>
          ...
          return (
            <div key={sys.label}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-slate-300 text-xs font-semibold">{sys.label}</span>
              ...
              <p className="text-slate-500 text-xs mt-1">{sys.impact}</p>
            </div>
          );
        })}
      </div>
      ...
      <p className="text-slate-600 text-xs">Estimates based on published NAD+ age-decline data. For educational purposes only.</p>
    </div>
  );
}
